...
- Hover a starting-five photo (tap it on a phone) to turn it into its 2K card. The first time, the cards open like a pack, walkouts and all; **Open the pack again** replays it. The tab on a card switches its badges and its attributes (the whole build, like 2K26's upgrade screen), and the switch on Yao's card brings Shaq in front. The biggest pulls bring the crowd up. Badges sit on 2K26's shields in their level's colour, Legend in 2K's red. **Full view** under every card opens it big in the middle of the screen, so the small numbers and badges read without zooming the browser; the arrows under it (or the arrow keys, or a swipe) go on through the five.
...
- Press **Play** on Malolos Rush in Crafts: a tiny three-lane runner through Malolos at sunset, playing a student from my capstone. Swipe to change lanes and swipe up to jump the barriers (drag with the mouse on a computer, or use the arrows and Space) and grab the coins. A jeepney ends the run (they honk when one's coming down your lane); clip a barrier and a guard gives chase, and clipping another before he drops back gets you caught. An ensaymada pulls the coins to you for a few seconds. On the way: Barasoain Church at 500 m, the Malolos Cathedral at 1000 m and Casa Real at 1500 m, each lighting up with bells or a fanfare as you reach it, while the sky turns to night. Your score is a point a metre and 10 a coin, and after a new best you can post it to the leaderboard. Add `?fps` to the address to see the frame rate. After a crash, **Share my run** makes a picture of the crash with your distance and a link back (the share sheet on a phone; copied on a computer, ready to paste into a chat). The link, `…/#malolos-rush`, opens the game straight away.
- In **On the court**, turn over my player's card under the intro: my NBA 2K26 MyCAREER build on a card like the game's builds screen, in G.O.A.T. gold. Its tab goes to the whole build and my jumper (Mochi, with its grades), then to all 35 badges and my 4 personality badges (point at one, or tap it, to read what it does; the icons are my own, one per badge group), then to my moves: my dribble and post animations. **Full view** under it (like under the starting five) opens the card big in the middle of the screen, with **Save as picture** (a 1080 x 1350 post: the share sheet on a phone, a download on a computer).
- The coach's board beside it draws three power forward plays: Back door (the cut ends in an easy layup), Horns (pick and pop) and 5-out post. Tap a tab under the board to draw one, or point at a word on the wall (tap it on a phone) to call up its play. Slow-mo draws it at a walk; a caption under the court says what each step is. **Guess the play** draws one with no name on it: pick it with the tabs and keep a streak going. The corner button opens the board big, in the middle of the screen. **Draw a play** opens it big in Coach mode: five players start 5 out; drag one to cut, or onto a teammate to set a screen; drag whoever has the ball to dribble, to the rim to drive, or onto a teammate to pass; tap them to shoot. Undo, Clear, and Replay draws your play again step by step (it's kept in your browser for next time). **Defense** puts an X on each player (drag one to guard tighter, or switch). **Share** makes a link: whoever opens it gets the board big with your play drawing itself (the share sheet on a phone, copied on a computer). **Save picture** makes a 1080 x 1080 picture of the play with its steps listed under it.
...
- `assets/` holds the web-ready copies the site uses: rule-of-thirds crops, lightly colour-graded, resized and stripped of metadata (no GPS). They're exported once, straight from the originals, as JPEG quality 97 with full colour detail (4:4:4), and those JPEGs are kept as the masters. The page itself loads a WebP twin of each (same size and name, `.webp`): quality 80, and 90 for the big 1600 px photos opened in the viewer. That's about a quarter of the weight with no difference you can see at the size they're shown, so the site loads and scrolls faster on phones. After exporting a new JPEG, make its WebP twin too (for example with `cwebp -q 80`).
- `assets/gallery/` holds the Snapshots gallery (JPEG masters and the WebP copies the page uses): each photo at 400, 800 and 1600 px (the page loads the small ones in the grid; a computer loads the 1600 px one when a photo is opened, a phone the 800 px one), plus a lighter 600 px copy that only phones use in the grid (quality 90), so the gallery loads faster on mobile data. Captions and order live in `gallery` in `content.js`.
- Edited a photo in Lightroom? Export a full-size JPEG into `images/` and re-crop it, or export straight to the matching file in `assets/` at the same size (`me-shop.jpg` 480×640, `me-cafe.jpg` 480×600, `me-camera.jpg` 480×480, `me-sofa.jpg` 640×480), then remake its `.webp` twin, which is the one the page shows.
