...
- `assets/gallery/` holds the Snapshots gallery (JPEG masters and the WebP copies the page uses): each photo at 400, 800 and 1600 px (the page loads the small ones in the grid and the 1600 px one only when a photo is opened), plus a lighter 600 px copy that only phones use in the grid (quality 90), so the gallery loads faster on mobile data. Captions and order live in `gallery` in `content.js`.
