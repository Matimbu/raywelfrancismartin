...
- **Intro**: my name with four of my photos around it, what I do, and where I'm based. The first visit in a session opens on an "RM." curtain.
...
Everything the site says is in `content.js`, grouped by section (`about`, `walls`, `story`, `crafts`, `hobbies`, `playlist`, `beliefs`, `youtube`, `gallery`, `links`). The comments in it explain each option.
...
- `assets/five/` holds the starting-five photos (480×640). `assets/five/cut/` holds the players cut out of them for the 2K cards: 600×800 transparent WebP in the card's shape, framed so a head, a hand or the ball breaks over the card's top edge (`cut` in `content.js`).
